...
func TestPathSimilarity(t *testing.T) {
	...
	// resolution of point boundary cells
	res := 20.0
	...
	score := PathSimilarity(res, res, path1, path1)
	if score != 1 {
		t.Errorf("the exact same path should return a score of 1")
	}

	score = PathSimilarity(res, res, path1, path3)
	if score != 0 {
		t.Errorf("two paths with non-overlapping boundaries should return a score of 0")
	}

	score = PathSimilarity(res, res, path1, path2)
	fmt.Printf("score for this last one is %v\n", score) 
	if !(score > 0 && score < 1) {
